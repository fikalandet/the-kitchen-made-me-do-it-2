...
interface EventsEditorProps {
  ...
  onSettingsChange: (settings: EventsSettings) => void;
}

export default function EventsEditor({ settings, onSettingsChange }: EventsEditorProps) {
  const [activeSubtitleIndex, setActiveSubtitleIndex] = useState(0);
  const [fadeIn, setFadeIn] = useState(true);

  const updateSetting = (key: keyof EventsSettings, value: any) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Evenemang</h3>
          <p className="text-sm text-gray-600">Anpassa inställningar för Evenemang-sektionen</p>
        </div>
      </div>
