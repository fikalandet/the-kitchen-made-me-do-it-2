...
interface FridgeMenuEditorProps {
  settings: FridgeMenuSettings;
  onSettingsChange: (settings: FridgeMenuSettings) => void;
}

export default function FridgeMenuEditor({ settings, onSettingsChange }: FridgeMenuEditorProps) {
  const [activeSubtitleIndex, setActiveSubtitleIndex] = useState(0);
  const [fadeIn, setFadeIn] = useState(true);

  const updateSetting = (key: keyof FridgeMenuSettings, value: any) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Kylskåpsmeny</h3>
          <p className="text-sm text-gray-600">Anpassa inställningar för Kylskåpsmeny-sektionen</p>
        </div>
      </div>
      ...
      <CollapsibleCard title="Produktbilder" defaultExpanded={true}>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Antal bilder i rad
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Layout för produktbilder
            </label>
            <div className="flex gap-3">
              <button
                onClick={() => updateSetting('layout', 'horizontal')}
                className={`px-4 py-2 rounded-lg border-2 transition-all ${
                  settings.layout === 'horizontal' || !settings.layout
                    ? 'border-[#56c5c5] bg-[#56c5c5] text-white'
                    : 'border-gray-300 hover:border-gray-400'
                }`}
              >
      ...
      <CollapsibleCard title="Preview" defaultExpanded={true}>
        ...
        >
          ...
          <div className={settings.layout === 'grid' ? 'grid gap-6' : 'flex gap-6 overflow-x-auto'} style={settings.layout === 'grid' ? { gridTemplateColumns: `repeat(${settings.cardsPerRow || 4}, 1fr)` } : {}}>
            {[1, 2, 3, 4].slice(0, settings.cardsPerRow || 4).map((i) => (
              <div key={i} className={`bg-white rounded-lg p-4 shadow ${settings.layout === 'horizontal' ? 'flex-shrink-0 w-64' : ''}`}>
