...
interface SchysstaDealsSettings {
...
interface SchysstaDealsEditorProps {
  settings: SchysstaDealsSettings;
  onSettingsChange: (settings: SchysstaDealsSettings) => void;
}

export default function SchysstaDealsEditor({ settings, onSettingsChange }: SchysstaDealsEditorProps) {
  const [activeSubtitleIndex, setActiveSubtitleIndex] = useState(0);
  const [fadeIn, setFadeIn] = useState(true);

  const updateSetting = (key: keyof SchysstaDealsSettings, value: any) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Schyssta deals</h3>
          <p className="text-sm text-gray-600">Anpassa inställningar för Schyssta deals-sektionen</p>
        </div>
      </div>
