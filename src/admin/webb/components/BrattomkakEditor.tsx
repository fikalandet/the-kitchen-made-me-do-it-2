...
interface BrattomkakEditorProps {
  settings: BrattomkakSettings;
  onSettingsChange: (settings: BrattomkakSettings) => void;
}

export default function BrattomkakEditor({ settings, onSettingsChange }: BrattomkakEditorProps) {
  const [activeSubtitleIndex, setActiveSubtitleIndex] = useState(0);
  const [fadeIn, setFadeIn] = useState(true);

  const updateSetting = (key: keyof BrattomkakSettings, value: any) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Bråttomkäk</h3>
          <p className="text-sm text-gray-600">Anpassa inställningar för Bråttomkäk-sektionen</p>
        </div>
      </div>
